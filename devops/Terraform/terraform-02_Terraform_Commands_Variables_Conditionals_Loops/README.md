...
resource "aws_s3_bucket" "tf-s3" {
  bucket = "serdar-tf-test-bucket-123"
}
```
...
- The `terraform apply -refresh-only` command is used to update the state file with the real-world infrastructure. This can be used to detect any drift from the last-known state, and to update the state file. First, check the current state of your resources with `terraform state list`. Then go to the AWS console and delete your S3 bucket `serdar-tf-test-bucket-addwhateveryouwant`. Display the state list again and refresh the state. Run the following commands.
...
resource "aws_s3_bucket" "tf-s3" {
  bucket = "serdar-tf-bucket-new"
  #bucket = "serdar-tf-bucket-123"
}
```
...
Changes to Outputs:
  + tf_example_private_ip = "xxxxxxxxxx"
...
variable "s3_bucket_name" {
  default = "serdar-s3-bucket-variable-123"
}
...
```tf
variable "s3_bucket_name" {
#   default     = "serdar-new-s3-bucket-123"
}
```
...
```bash
terraform plan -var="s3_bucket_name=serdar-new-s3-bucket-2"
```
...
```bash
export TF_VAR_s3_bucket_name=serdar-env-varible-bucket
terraform plan
```
...
- Create a file name `serdar.tfvars`. Add the followings.

```bash
s3_bucket_name = "serdar-tfvar-bucket"
```

- Run the command below.

```bash
terraform plan --var-file="serdar.tfvars"
```

- Create a file named `serdar.auto.tfvars`. Add the followings.

```bash
s3_bucket_name = "serdar-auto-tfvar-bucket"
```
